...
from torch_points3d.modules.multimodal.modules import MultimodalBlockDown, \
    MVAttentionUnimodalBranch, IdentityBranch
...
# ------------------------ UNWRAPPED UNET BASE ----------------------- #
### Adapted by Feng
class MVAttentionUnwrappedUnetBasedModel(BaseModel, ABC):
    ...
    def __init__(self, opt, model_type, dataset: BaseDataset, modules_lib):
        ...
        * up_modules: Contains all the up module
        ...
        """
        opt = copy.deepcopy(opt)
        super(MVAttentionUnwrappedUnetBasedModel, self).__init__(opt)
        self._spatial_ops_dict = {
            "neighbour_finder": [], "sampler": [], "upsample_op": []}
        ...
        if self.is_multimodal:
            ...
            for m in self.modalities:
                ...
                for i, idx in enumerate(b_idx):
                    ...
                    branches[idx][m] = MVAttentionUnimodalBranch(
...
def MVAttentionSparseConv3d(
    ...
    **kwargs
    ...
    factory = MVAttentionSparseConv3dFactory(
...
class MVAttentionSparseConv3dFactory(ModelFactory):
    def _build_unet(self):
        ...
        return MVAttentionSparseConv3dUnet(model_config, None, None, modules_lib,
                                **self.kwargs)
...
class MVAttentionBaseSparseConv3d(MVAttentionUnwrappedUnetBasedModel):
    CONV_TYPE = "sparse"
...
class MVAttentionSparseConv3dEncoder(MVAttentionBaseSparseConv3d):
    def forward(self, data, *args, **kwargs):
        """
        Parameters:
        -----------
        ...
        data:
            - x [1, output_nc]
        """
        print("_set_input call in MVAttentionSparseConv3dEncoder forward", flush=True)
        raise NotImplementedError
...
class MVAttentionSparseConv3dUnet(MVAttentionBaseSparseConv3d):
    def forward(self, data, *args, **kwargs):
        """Run forward pass.
...
class MVAttentionAPIModel(BaseModel):
    def __init__(self, option, model_type, dataset, modules):
        # call the initialization method of MVAttentionUnetBasedModel
        super().__init__(option)
        self._weight_classes = dataset.weight_classes
        
        option['backbone']['transformer']['n_classes'] = dataset.num_classes
        self.backbone = MVAttentionSparseConv3d(
