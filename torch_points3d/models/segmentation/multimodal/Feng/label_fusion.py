...
from torch_points3d.modules.multimodal.modules import MultimodalBlockDown, \
    LabelFusionUnimodalBranch, IdentityBranch
...
# ------------------------ UNWRAPPED UNET BASE ----------------------- #
### Adapted by Feng
class LabelFusionUnwrappedUnetBasedModel(BaseModel, ABC):
    ...
    def __init__(self, opt, model_type, dataset: BaseDataset, modules_lib):
        ...
        * up_modules: Contains all the up module
        ...
        """
        opt = copy.deepcopy(opt)
        super(LabelFusionUnwrappedUnetBasedModel, self).__init__(opt)
        self._spatial_ops_dict = {
            "neighbour_finder": [], "sampler": [], "upsample_op": []}
        ...
        if self.is_multimodal:
            ...
            for m in self.modalities:
                ...
                for i, idx in enumerate(b_idx):
                    ...
                    branches[idx][m] = LabelFusionUnimodalBranch(
...
def LabelFusionSparseConv3d(
    ...
    **kwargs
    ...
    factory = LabelFusionSparseConv3dFactory(
...
class LabelFusionSparseConv3dFactory(ModelFactory):
    def _build_unet(self):
        ...
        return LabelFusionSparseConv3dUnet(model_config, None, None, modules_lib,
                                **self.kwargs)
...
class LabelFusionBaseSparseConv3d(LabelFusionUnwrappedUnetBasedModel):
    CONV_TYPE = "sparse"
...
class LabelFusionSparseConv3dEncoder(LabelFusionBaseSparseConv3d):
    def forward(self, data, *args, **kwargs):
        """
        Parameters:
        -----------
        ...
        data:
            - x [1, output_nc]
        """
        print("_set_input call in LabelFusionSparseConv3dEncoder forward", flush=True)
        raise NotImplementedError
...
class LabelFusionSparseConv3dUnet(LabelFusionBaseSparseConv3d):
    def forward(self, data, *args, **kwargs):
        """Run forward pass.
...
class LabelFusionAPIModel(BaseModel):
    def __init__(self, option, model_type, dataset, modules):
        # call the initialization method of LabelFusionUnetBasedModel
        super().__init__(option)
        self._weight_classes = dataset.weight_classes
        
        option['backbone']['transformer']['n_classes'] = dataset.num_classes
        self.backbone = LabelFusionSparseConv3d(
