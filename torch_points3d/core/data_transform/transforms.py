...
class SphereSampling:
    ...
    def __call__(self, data):
        ...
        if not hasattr(data, self.KDTREE_KEY):
#             print("data not has attry KDTREE_KEY")
#             print('np.isinf(X).any(): ', np.isinf(np.asarray(data.pos)).any())
#             print('np.isnan(X).any(): ', np.isnan(np.asarray(data.pos)).any())
#             print("np.isfinite(X).all()", np.isfinite(np.asarray(data.pos)).all())
#             print('max val: ', np.max(np.asarray(data.pos)))
            
            tree = KDTree(np.asarray(data.pos), leaf_size=50)
            setattr(data, self.KDTREE_KEY, tree)
        else:
#             print("data already has attry KDTREE_KEY")
            tree = getattr(data, self.KDTREE_KEY)
#             print('np.isinf(X).any(): ', np.isinf(tree.data).any())
#             print('np.isnan(X).any(): ', np.isnan(tree.data).any())
#             print("np.isfinite(X).all()", np.isfinite(tree.data).all())
#             print('max val: ', np.max(tree.data))
            

#         print("self._centre: ", self._centre)
